...
--
-- ⚠️  Atomicité : drop + add réunis dans UN SEUL `alter table`. Un statement
--     ALTER TABLE à sous-commandes multiples s'exécute en un bloc atomique —
--     il n'existe à aucun instant de fenêtre où la table serait sans
--     contrainte de statut, et ce indépendamment du wrapping transactionnel
--     du migrateur (pas de BEGIN/COMMIT explicite qui s'imbriquerait avec
--     la transaction Supabase).
--
-- Pré-requis vérifié sur prod (2026-06-21) : `select status, count(*) ...`
--   → active(9), cancelled(1), expired(1). Aucune valeur orpheline hors des
--   5 valeurs existantes → le recreate de contrainte passera sans rejet.
